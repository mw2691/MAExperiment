...
public class FileWriteManagement
{
    ...
    static string resultsFolderName = "ResultFiles";
    static string trialOrderFilePath = Path.Combine(Directory.GetCurrentDirectory(), trialOrderFolderName);
    static string resultsFilePath = Path.Combine(Directory.GetCurrentDirectory(), resultsFolderName);
    ...
    public static void CreateDirectory(string participantID, string folderName)
    {
        string currentPath = Path.Combine(Directory.GetCurrentDirectory());
        currentPath = currentPath + "/" +  folderName + "/" + participantID;
    ...
    public static List<string> GetAllResultFiles(string participantID)
    {
        resultsFilePath = Path.Combine(Directory.GetCurrentDirectory(), resultsFolderName);
        List<string> allResultFiles = new List<string>();
        resultsFilePath += "/" + participantID;
        string[] resultFiles = Directory.GetFiles(resultsFilePath, "*.log").Select(Path.GetFileName).ToArray();

        foreach (var file in resultFiles)
        {
            if (file.Contains("_train_"))
            {
                continue;
            }
            else
    ...
    public static int GetTrialOrderFileLength(string participantID)
    {
        string fullPath = trialOrderFilePath + "/" + GetTrialOrderFileName(participantID);
        string[] arrLine = File.ReadAllLines(fullPath);
    ...
    //public static string GetResultLine(string participantID, int lineNumber)
    //{
    //    string fullPath = trialOrderFilePath + "/" + GetTrialOrderFileName(participantID);
    //    string[] arrLine = File.ReadAllLines(fullPath);
    //    return arrLine[lineNumber];
